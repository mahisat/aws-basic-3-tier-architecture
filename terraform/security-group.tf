resource "aws_security_group" "web_server_security_group" {
  name        = "${var.project}-web-server-sg"
  description = "Frontend EC2: HTTP from the internet"
...
resource "aws_security_group" "ssh_security_group" {
  ...
  description = "SSH to frontend EC2 only (prefer SSM where possible)"
  vpc_id      = aws_vpc.main.id
...
resource "aws_security_group" "backend_server_security_group" {
  ...
  description = "Backend API reachable only from the frontend security group"
  vpc_id      = aws_vpc.main.id
